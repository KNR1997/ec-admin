...
// Function to convert ISO 8601 datetime to YYYY-MM-DD format
function convertToDateString(isoString: any) {
  if (!isoString) return null; // Handle cases where the date might be null or undefined
...
export function getProductInputValues(
  ...
  console.log('getProductInputValues: ', values)
    ...
    ...(product_type?.value === ProductType?.Simple && {
      batches: {
        upsert: batches?.map((batch) => ({
          ...
          expiry_date: convertToDateString(batch?.expiry_date),
          manufacture_date: convertToDateString(batch?.manufacture_date),
        })),
        delete: initialValues?.batches
          ?.map((initialBatch: Batch) => {
            // @ts-ignore
            const find = batches?.find(
              (batch: Batch) => batch?.id === initialBatch?.id,
            );
            if (!find) {
              return initialBatch?.id;
            }
          })
